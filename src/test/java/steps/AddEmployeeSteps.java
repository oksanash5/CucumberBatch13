   ...
   String idFromUI;
    ...
    public void user_adds_multiple_employees_and_verify_they_are_added(DataTable dataTable) throws InterruptedException {
        ...
        for (Map<String, String> emp : employeeNames) {
            ...
            //we don't want to execute background multiple time
            //we will click on add employee option to add another employee till
            //the time we are getting fresh value from feature file
            Thread.sleep(2000);
            click(dash.addEmployeeOption);
            Thread.sleep(2000);
    ...
    public void user_adds_multiple_employees_from_excel_file_using_sheet_and_verify_the_employee_has_added(String sheetName) throws InterruptedException {
        ...
        while (itr.hasNext()){
            ...
            // if we are not able to click regular clicks  (click(dash.employeeListOption);)
            // it shows like NoSuchElementException for the next element emp.idEmployeeSearch
            // we use java script executor click -jsClick
